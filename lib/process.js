...
export function crunchOrders(orders, retailCategorySet = null) {
  ...
  let grossSales = 0, taxTotal = 0, discountTotal = 0, tipTotal = 0, serviceChargeTotal = 0;
  const itemMap = {};
  const discountMap = {};
  ...
  for (const order of live) {
    for (const check of (order.checks || [])) {
      if (retailCategorySet === null) {
        // check.amount = pre-tax total (totalAmount - taxAmount), already computed by Toast
        grossSales    += check.amount         || 0;
        taxTotal      += check.taxAmount      || 0;
        discountTotal += check.discountAmount || 0;
        // Service charges are inside amount but not menu item revenue
        for (const sc of (check.appliedServiceCharges || [])) {
          serviceChargeTotal += sc.chargeAmount || 0;
        }
      ...
      for (const sel of (check.selections || [])) {
        ...
        if (retailCategorySet !== null) {
          ...
          grossSales += (sel.price || 0); // sel.price is already line total (unit × qty)
        }
        ...
        itemMap[key].qty     += qty;
        itemMap[key].revenue += (sel.price || 0); // sel.price is already line total
      }
    }
  }

  const appliedDiscountTotal = Object.values(discountMap).reduce((s, d) => s + d.total, 0);
  // check.amount is already post-discount pre-tax — no further deduction needed
  ...
  const allItemsList = Object.values(itemMap);
  const totalItemQty = allItemsList.reduce((s, i) => s + i.qty, 0);
  const totalItemRevenue = allItemsList.reduce((s, i) => s + i.revenue, 0);
  const topItems   = allItemsList.sort((a, b) => b.qty - a.qty).slice(0, 20);
  const discounts  = Object.values(discountMap).sort((a, b) => b.total - a.total);

  return { grossSales, netSales, taxTotal, discountTotal: appliedDiscountTotal, tipTotal, orderCount, avgCheck, topItems, totalItemQty, totalItemRevenue, discounts };
}
  ...
  for (const order of live) {
    for (const check of (order.checks || [])) {
      for (const sel of (check.selections || [])) {
        ...
        byCategory[cat][key].revenue += (sel.price || 0); // sel.price is already line total
      }
    }
  }
  const result = {};
...
export function crunchLabor(timeEntries, employeeMap = {}) {
  ...
  // Gross shift minus any taken (unpaid) break time
  const netHours = entry => {
    const gross = (new Date(entry.outDate) - new Date(entry.inDate)) / 3_600_000;
    const breakTime = (entry.breaks || [])
      .filter(b => !b.missed && !b.waived && b.inDate && b.outDate)
      .reduce((s, b) => s + (new Date(b.outDate) - new Date(b.inDate)) / 3_600_000, 0);
    return gross - breakTime;
  };
  ...
  for (const entry of (timeEntries || [])) {
    ...
    const validBreaks = breaks.filter(b => !b.missed && !b.waived && b.inDate && b.outDate);
    const hadValidBreak = validBreaks.length > 0;

    // Check if break was taken after hour 5 (CA law: must break before end of 5th hour)
    let breakHour = null;
    if (hadValidBreak) {
      for (const b of validBreaks) {
        const hrs = (new Date(b.inDate) - new Date(entry.inDate)) / 3_600_000;
        if (hrs > 5.5) { breakHour = +hrs.toFixed(1); break; }
      }
      if (breakHour === null) continue; // break was on time, skip
    }
    ...
    missedBreaks.push({
      ...
      noBreak: !hadValidBreak,
      lateBreak: hadValidBreak,
      breakHour,
    });
  }
  ...
  const totalLaborCost = (timeEntries || []).reduce((sum, e) => {
    ...
    return sum + netHours(e) * (e.hourlyWage || 0);
  }, 0);
...
export function buildTimeSeries(orders, timeEntries, employeeMap = {}) {
  ...
  const fmtTimePT = iso => new Date(iso).toLocaleTimeString('en-US', {
    timeZone: 'America/Los_Angeles', hour: 'numeric', minute: '2-digit',
  });
  const empName = entry => {
    const id = entry.employee?.guid || entry.employeeReference?.guid;
    const emp = id && employeeMap[id];
    return emp ? [emp.firstName, emp.lastName].filter(Boolean).join(' ') : null;
  };

  let firstArrival = null, lastDeparture = null;
  for (const entry of timeEntries) {
    if (!entry.inDate) continue;
    if (!firstArrival || +new Date(entry.inDate) < +new Date(firstArrival.iso)) {
      firstArrival = { iso: entry.inDate, time: fmtTimePT(entry.inDate), name: empName(entry) };
    }
    if (entry.outDate && (!lastDeparture || +new Date(entry.outDate) > +new Date(lastDeparture.iso))) {
      lastDeparture = { iso: entry.outDate, time: fmtTimePT(entry.outDate), name: empName(entry) };
    }
  }
