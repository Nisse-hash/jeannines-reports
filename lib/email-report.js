  ...
  <table width="100%" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <td align="right" valign="top" style="padding-left:12px;">
        ...
        ${events?.national?.length ? events.national.slice(0, 2).map(e => `<div style="color:#C9A84C;font-size:11px;font-weight:600;margin-top:6px;text-align:right;line-height:1.4;">✦ ${e}</div>`).join('') : ''}
      </td>
    </tr>
  </table>
</td></tr>
...
<tr><td style="padding:28px 0 0;">
  <div style="height:2px;background:linear-gradient(90deg,transparent,${locColor},transparent);border-radius:1px;"></div>
      ...
      const locNational = events?.national || [];
      const locSpecific = events?.byLocation?.[loc.guid]?.today || [];
      if (locNational.length || locSpecific.length) {
        html += `<tr><td style="padding:0 0 8px;">${eventBanner({ national: locNational, local: locSpecific })}</td></tr>`;
      }
      html += `
...
<tr><td style="height:20px;"></td></tr>`;
    ...
    // Upcoming events — tomorrow and day after
    const tomorrowEvents = events?.byLocation?.[loc.guid]?.tomorrow || [];
    const dayAfterEvents = events?.byLocation?.[loc.guid]?.dayAfter || [];
    if (tomorrowEvents.length || dayAfterEvents.length) {
...
<tr><td style="padding:8px 0 4px;">
  <table width="100%" cellpadding="0" cellspacing="0" bgcolor="#111118" style="background-color:#111118;border-radius:6px;border:1px solid #2a2a35;">
    <tr><td style="padding:10px 14px;">
      <div style="color:#444455;font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:6px;">Upcoming</div>
      ${tomorrowEvents.map(e => `<div style="color:#888888;font-size:12px;line-height:1.5;margin-bottom:2px;"><span style="color:#C9A84C;font-size:10px;font-weight:700;margin-right:6px;">${tomorrowLabel}</span>${e}</div>`).join('')}
      ${dayAfterEvents.map(e => `<div style="color:#888888;font-size:12px;line-height:1.5;margin-bottom:2px;"><span style="color:#666666;font-size:10px;font-weight:700;margin-right:6px;">${dayAfterLabel}</span>${e}</div>`).join('')}
    </td></tr>
  </table>
</td></tr>`;
    }
