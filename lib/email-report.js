...
<tr><td style="height:20px;"></td></tr>`;
  ...
  // CONSOLIDATED EVENTS — holidays + graduation only
  {
    const todayAll  = [...(events?.today?.holidays  || []), ...(events?.today?.local  || [])];
    const tomAll    = [...(events?.tomorrow?.holidays || [])];
    const dayAfAll  = [...(events?.dayAfter?.holidays || [])];
    if (todayAll.length || tomAll.length || dayAfAll.length) {
  ...
  <table width="100%" cellpadding="0" cellspacing="0" bgcolor="#1C1600" style="background-color:#1C1600;border-radius:6px;border:1px solid #C9A84C;">
    <tr><td style="padding:12px 16px;">
      ${todayAll.length ? `<div style="color:#C9A84C;font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:6px;">Today</div>
      ${todayAll.map(e => `<div style="color:#F0D878;font-size:12px;font-weight:600;line-height:1.6;">✦ ${e}</div>`).join('')}` : ''}
      ${tomAll.length || dayAfAll.length ? `<div style="color:#555555;font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;margin-top:${todayAll.length ? '10px' : '0'};margin-bottom:4px;">Watch for</div>
      ${tomAll.map(e => `<div style="color:#888888;font-size:12px;line-height:1.6;"><span style="color:#C9A84C;font-size:10px;font-weight:700;margin-right:6px;">${tomorrowLabel}</span>${e}</div>`).join('')}
      ${dayAfAll.map(e => `<div style="color:#888888;font-size:12px;line-height:1.6;"><span style="color:#666666;font-size:10px;font-weight:700;margin-right:6px;">${dayAfterLabel}</span>${e}</div>`).join('')}` : ''}
    </td></tr>
  </table>
</td></tr>`;
    }
