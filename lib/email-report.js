...
export async function generateEmailHtml(data, displayDate, reportUrl) {
  ...
  // Chart GIF + peak hour fallback
  const gifBuffer = await generateChartGif(ts);
  const gifBase64 = gifBuffer ? gifBuffer.toString('base64') : null;
...
<tr><td style="height:10px;"></td></tr>`;
  ...
  // DAY AT A GLANCE — animated GIF if available, text fallback otherwise
  if (gifBase64 || peakLine) {
    html += `
<!-- DAY AT A GLANCE -->
<tr><td bgcolor="#13131A" style="background-color:#13131A;border-radius:8px;padding:16px 16px ${gifBase64 ? '12px' : '16px'};">
  <div style="color:#666666;font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:10px;">Day at a Glance</div>`;
    if (gifBase64) {
      html += `
  <img src="data:image/gif;base64,${gifBase64}" width="508" alt="Day at a glance chart" style="display:block;max-width:100%;border-radius:4px;">
  ...
  <div style="margin-top:10px;"><a href="${reportUrl}" style="color:#C9A84C;text-decoration:none;font-size:13px;font-weight:600;">See full chart →</a></div>`;
    }
    html += `
</td></tr>
<tr><td style="height:10px;"></td></tr>`;
