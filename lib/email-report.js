...
<tr><td style="height:20px;"></td></tr>`;
  ...
  // CONSOLIDATED EVENTS — today + upcoming, all locations merged
  {
    const allToday = [...new Set([
      ...(events?.national || []),
      ...Object.values(events?.byLocation || {}).flatMap(d => d.today || []),
    ])];
    const allTomorrow = [...new Set(Object.values(events?.byLocation || {}).flatMap(d => d.tomorrow || []))];
    const allDayAfter = [...new Set(Object.values(events?.byLocation || {}).flatMap(d => d.dayAfter || []))];
    if (allToday.length || allTomorrow.length || allDayAfter.length) {
      const tomorrowLabel = dayOfWeek(addDays(displayDate, 1)).slice(0, 3).toUpperCase();
      const dayAfterLabel = dayOfWeek(addDays(displayDate, 2)).slice(0, 3).toUpperCase();
      html += `<tr><td style="padding:0 0 16px;">
  <table width="100%" cellpadding="0" cellspacing="0" bgcolor="#1C1600" style="background-color:#1C1600;border-radius:6px;border:1px solid #C9A84C;">
    <tr><td style="padding:12px 16px;">
      ${allToday.length ? `<div style="color:#C9A84C;font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:6px;">Today</div>
      ${allToday.map(e => `<div style="color:#F0D878;font-size:12px;font-weight:600;line-height:1.6;">✦ ${e}</div>`).join('')}` : ''}
      ${allTomorrow.length || allDayAfter.length ? `<div style="color:#555555;font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;margin-top:${allToday.length ? '10px' : '0'};margin-bottom:6px;">Watch for</div>
      ${allTomorrow.map(e => `<div style="color:#888888;font-size:12px;line-height:1.6;"><span style="color:#C9A84C;font-size:10px;font-weight:700;margin-right:6px;">${tomorrowLabel}</span>${e}</div>`).join('')}
      ${allDayAfter.map(e => `<div style="color:#888888;font-size:12px;line-height:1.6;"><span style="color:#666666;font-size:10px;font-weight:700;margin-right:6px;">${dayAfterLabel}</span>${e}</div>`).join('')}` : ''}
    </td></tr>
  </table>
</td></tr>`;
    }
...
<tr><td style="padding-top:${li === 0 ? '0' : '16px'};padding-bottom:14px;">
...
<tr><td>
  <table width="100%" cellpadding="0" cellspacing="0"><tr>
    <td width="4" valign="top" bgcolor="${locColor}" style="background-color:${locColor};border-radius:3px;">&nbsp;</td>
    <td style="padding-left:12px;"><table width="100%" cellpadding="0" cellspacing="0">`;
...
<tr><td style="height:10px;"></td></tr>`;
    ...
    const managerName = LOCATION_MANAGERS[loc.guid];
    const laborNote = `<span style="font-size:9px;font-weight:400;color:#888888;line-height:1.05;">Restaurant Staff<br>Restaurant Cleaning Staff<br>Bakery (9 people)<br>Maintenance Staff</span>${managerName ? `<div style="margin-top:6px;padding-top:5px;border-top:1px solid #222222;font-size:10px;color:#555555;font-weight:600;">${managerName}</div>` : ''}`;
    html += `
<tr><td>
  <table width="100%" cellpadding="0" cellspacing="0">
