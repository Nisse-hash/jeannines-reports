...
export async function generateEmailHtml(data, displayDate, reportUrl) {
  ...
  // Chart: get hosted URL from QuickChart (Gmail blocks base64 data URIs)
  const chartUrl = await generateChartGif(ts);
...
<tr><td style="height:10px;"></td></tr>`;
  ...
  if (labor.overtime?.length) {
    html += `
<!-- OVERTIME TABLE -->
<tr><td bgcolor="#13131A" style="background-color:#13131A;border-radius:8px;padding:16px 18px 10px;">
  <div style="color:#FF6B6B;font-size:12px;font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:12px;">Overtime Breakdown</div>
  <table width="100%" cellpadding="0" cellspacing="0">
    <tr>
      <td style="color:#444444;font-size:10px;font-weight:700;padding-bottom:8px;border-bottom:1px solid #2a1010;">EMPLOYEE</td>
      <td align="right" width="55" style="color:#444444;font-size:10px;font-weight:700;padding-bottom:8px;border-bottom:1px solid #2a1010;">TOTAL</td>
      <td align="right" width="55" style="color:#444444;font-size:10px;font-weight:700;padding-bottom:8px;border-bottom:1px solid #2a1010;">OT HRS</td>
      <td align="right" width="70" style="color:#444444;font-size:10px;font-weight:700;padding-bottom:8px;border-bottom:1px solid #2a1010;">EXTRA</td>
    </tr>`;
    for (const emp of labor.overtime) {
      const dtBadge = emp.doubleTime
        ? `<span style="display:inline-block;font-size:9px;font-weight:700;padding:1px 5px;border-radius:3px;color:#FF6B6B;background-color:rgba(255,107,107,.12);border:1px solid rgba(255,107,107,.3);margin-left:5px;">2x</span>`
        : '';
      html += `
    <tr>
      <td style="color:#FFFFFF;font-size:13px;padding:7px 0;">${emp.name}${dtBadge}</td>
      <td align="right" style="color:#999999;font-size:13px;padding:7px 4px;white-space:nowrap;">${emp.hours}h</td>
      <td align="right" style="color:#FF6B6B;font-size:13px;padding:7px 4px;white-space:nowrap;">+${emp.otHours}h</td>
      <td align="right" style="color:#FF6B6B;font-size:13px;padding:7px 0;white-space:nowrap;">${fmt$(emp.extraCost)}</td>
    </tr>`;
    }
    html += `
    <tr>
      <td colspan="3" style="color:#666666;font-size:11px;padding:8px 0 0;border-top:1px solid #2a1010;">Total extra labor cost</td>
      <td align="right" style="color:#FF6B6B;font-size:13px;font-weight:700;padding:8px 0 0;border-top:1px solid #2a1010;white-space:nowrap;">${fmt$(labor.totalOtCost)}</td>
    </tr>
  </table>
</td></tr>
<tr><td style="height:10px;"></td></tr>`;
...
<tr><td bgcolor="#13131A" style="background-color:#13131A;border-radius:8px;padding:18px 18px 10px;">
  ...
  <table width="100%" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <td align="right" width="90" style="color:#444444;font-size:10px;font-weight:700;padding-bottom:8px;border-bottom:1px solid #222222;">REVENUE</td>
    </tr>`;
    sales.topItems.slice(0, 10).forEach((item, i) => {
      html += `
    <tr>
      <td style="color:#555555;font-size:14px;padding:8px 4px 8px 0;">${i + 1}</td>
      <td style="color:#FFFFFF;font-size:14px;padding:8px 8px;">${item.name}</td>
      <td align="right" style="color:#FFFFFF;font-size:14px;padding:8px 4px;white-space:nowrap;">${item.qty}</td>
      <td align="right" style="color:#C9A84C;font-size:14px;padding:8px 0 8px 4px;white-space:nowrap;">${fmt$(item.revenue)}</td>
    </tr>`;
    });
    html += `
  </table>
...
<tr><td bgcolor="#13131A" style="background-color:#13131A;border-radius:8px;padding:18px 18px 10px;">
  <table width="100%" cellpadding="0" cellspacing="0">
    <tr>
      <td style="color:#444444;font-size:10px;font-weight:700;padding-bottom:8px;border-bottom:1px solid #222222;">ITEM</td>
      <td align="right" width="50" style="color:#444444;font-size:10px;font-weight:700;padding-bottom:8px;border-bottom:1px solid #222222;">QTY</td>
      <td align="right" width="90" style="color:#444444;font-size:10px;font-weight:700;padding-bottom:8px;border-bottom:1px solid #222222;">REVENUE</td>
    </tr>`;
      allRetailItems.slice(0, 15).forEach(item => {
        html += `
    <tr>
      <td style="color:#FFFFFF;font-size:14px;padding:8px 8px 8px 0;">${item.name}</td>
      <td align="right" style="color:#FFFFFF;font-size:14px;padding:8px 4px;white-space:nowrap;">${item.qty}</td>
      <td align="right" style="color:#C9A84C;font-size:14px;padding:8px 0 8px 4px;white-space:nowrap;">${fmt$(item.revenue)}</td>
    </tr>`;
      });
