...
function chartSection(ts) {
  ...
  <div class="chart-outer">
    ...
    <div class="chart-wrap">
      ...
      <div id="${panelId}" class="staff-panel-col">
        <div class="sp-idle">Hover chart<br>to see who's<br>on the floor</div>
      </div>
    </div>
  </div>
  <script>
  (function(){
    ...
    function updatePanel(idx) {
      ...
      if (!panel) return;
      if (idx < 0) {
        panel.innerHTML = '<div class="sp-idle">Hover chart<br>to see who\'s<br>on the floor</div>';
        return;
      }
      const names = staff[idx] || [];
      panel.innerHTML = '<div class="sp-time">' + labels[idx] + '</div>'
        + (names.length
          ? names.map(n => '<div class="sp-name">· ' + n + '</div>').join('')
          : '<div class="sp-idle">No staff data</div>');
    }
...
.chart-canvas-col{flex:1;min-width:0;padding:1rem;position:relative}
.staff-panel-col{width:180px;flex-shrink:0;border-left:1px solid #1A1A24;padding:.75rem;overflow-y:auto;display:flex;flex-direction:column;background:#0A0A0F}
...
.sp-idle{font-size:.7rem;color:#333;margin:auto;text-align:center;line-height:1.6}
