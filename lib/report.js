...
function retailTopItemsSection(retailLocations) {
  ...
  // Merge across all locations
  const mergedByCategory = {};
  for (const loc of retailLocations) {
    for (const [cat, items] of Object.entries(loc.topItemsByCategory || {})) {
      if (!mergedByCategory[cat]) mergedByCategory[cat] = {};
      for (const item of items) {
        if (!mergedByCategory[cat][item.name]) mergedByCategory[cat][item.name] = { name: item.name, qty: 0, revenue: 0 };
        mergedByCategory[cat][item.name].qty     += item.qty;
        mergedByCategory[cat][item.name].revenue += item.revenue;
      }
    }
  }

  const foodItems    = Object.values(mergedByCategory['Retail Food']     || {}).sort((a, b) => b.qty - a.qty).slice(0, 20);
  const nonFoodItems = Object.values(mergedByCategory['Retail Non-Food'] || {}).sort((a, b) => b.qty - a.qty).slice(0, 20);

  if (!foodItems.length && !nonFoodItems.length) return '';

  const allMap = {};
  for (const item of [...foodItems, ...nonFoodItems]) {
    if (!allMap[item.name]) allMap[item.name] = { name: item.name, qty: 0, revenue: 0 };
    allMap[item.name].qty     += item.qty;
    allMap[item.name].revenue += item.revenue;
  }
  ...
  const allTable     = allItems.length     ? topItemsTable(allItems)     : '<p class="empty">No retail items sold.</p>';
  ...
  <div class="location-block" style="border:1px solid rgba(78,205,196,.2)">
    ...
    <div class="retail-toggle">
      <button class="rtgl-btn active" data-view="split" onclick="setRetailView('split')">Food &amp; Non-Food</button>
      ...
      <button class="rtgl-btn" data-view="all" onclick="setRetailView('all')">All</button>
    </div>

    <div id="retail-cols" class="retail-grid">
    ...
    <div id="retail-all-view" style="display:none">
      ${allTable}
    </div>
  </div>`;
}
...
export function render({ locations, totals, eightySixedItems = [], retailLocations = [] }, displayDate) {
...
/* Retail toggle */
.retail-toggle{display:flex;align-items:center;gap:.375rem;margin-bottom:1.25rem;flex-wrap:wrap}
