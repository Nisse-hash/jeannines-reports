...
function retailAllTable(items) {
  ...
  return `
    <div class="table-toolbar">
      <span class="sort-label">Sort by</span>
      <button class="sort-btn active" onclick="sortRetailAll('${id}','qty')">Quantity</button>
      <button class="sort-btn" onclick="sortRetailAll('${id}','revenue')">Revenue</button>
    </div>
    <table class="data-table" id="${id}">
...
function retailTopItemsSection(retailLocations) {
  ...
  const allTable     = retailAllTable(allItems);
    ...
    new Chart(document.getElementById('${id}'), {
      ...
      options: {
        ...
        plugins:{
          ...
          tooltip:{
            backgroundColor:'#13131A', borderColor:'#2A2A3A', borderWidth:1,
            ...
            callbacks:{
              ...
              afterBody: items => {
                ...
                const names = idx !== undefined ? (staff[idx] || []) : [];
                if (!names.length) return [];
                return ['', '● On the floor:'].concat(names.map(n => '   ' + n));
              },
            }
          },
        }
