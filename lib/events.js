...
function cleanAnswer(answer, town) {
  ...
  let cleaned = answer
    .replace(/^On\s+[A-Z][a-z]+\s+\d{1,2},\s+\d{4},?\s*/i, '')  // strip "On June 13, 2026, "
    .trim();

  // Split into sentences, drop noise sentences, keep first useful one
  const sentences = cleaned.split(/(?<=[.!?])\s+/);
  const useful = sentences.filter(s => !NOISE_SENTENCE.test(s));
  cleaned = useful.slice(0, 1).join(' ').trim();
...
async function fetchTownEventsForDate(dateStr, town) {
  ...
  const queries = [
    `events happening in ${town} California on ${humanDate}`,
  ];
