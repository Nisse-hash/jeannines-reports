...
export async function publishChartImage(buffer, displayDate) {
  const token = process.env.GITHUB_TOKEN;
  const repo  = process.env.JEANNINES_GITHUB_REPORT_REPO || 'jeannines-reports';
  const octokit = new Octokit({ auth: token });
  const owner = await getOwner(octokit);

  const filePath = `reports/chart-${displayDate}.png`;
  ...
  await octokit.repos.createOrUpdateFileContents({
    owner, repo, path: filePath,
    message: `Chart ${displayDate}`,
    content,
    ...(sha ? { sha } : {}),
  });

  const vercelBase = process.env.JEANNINES_REPORT_URL_BASE || `https://${repo}.vercel.app`;
  return `${vercelBase}/reports/chart-${displayDate}.png`;
}
