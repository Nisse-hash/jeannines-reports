...
export async function generateChartGif(timeSeries) {
  ...
  // Crop: end of service = last slot where BOTH revenue AND staff exist
  // Then extend to when headcount actually hits 0 (last employee out)
  // Cap: never go more than 20 slots past end of service (avoids next-day anomalies)
  let first = 0, last = n - 1;
  for (let i = 0; i < n; i++) { if (revenue[i] > 0 || headcount[i] > 0) { first = i; break; } }
  let lastActive = -1;
  for (let i = n - 1; i >= 0; i--) {
    if (revenue[i] > 0 && headcount[i] > 0) { lastActive = i; break; }
  }
  if (lastActive < 0) {
    for (let i = n - 1; i >= 0; i--) { if (revenue[i] > 0) { lastActive = i; break; } }
  }
  const cap = lastActive >= 0 ? lastActive + 20 : n - 1;
  let lastEmployee = lastActive >= 0 ? lastActive : 0;
  for (let i = Math.min(cap, n - 1); i >= 0; i--) {
    if (headcount[i] > 0) { lastEmployee = i; break; }
  }
  last = lastEmployee;
  ...
  for (let fi = 0; fi < N; fi++) {
    ...
    // Revenue — bold, prominent
    ctx.fillStyle = '#FFFFFF';
    ctx.font = 'bold 15px Arial';
    ctx.fillText(fmtRevenue(rev), PL, 63);

    // Labor — dim red
    ctx.fillStyle = '#AA4444';
    ctx.font = '9px Arial';
    ctx.fillText('$' + labor.toFixed(2) + ' labor', PL, 78);

    // Separator
    ctx.fillStyle = '#1E1E30';
    ctx.fillRect(PL, 85, W - PL - 6, 1);

    // Staff names — 2 columns so all names fit at a comfortable size
    const namesTop = 98;
    const availH   = H - 8 - namesTop;
    const colW     = Math.floor((W - PL - 6) / 2);
    const rows     = Math.ceil(names.length / 2);
    if (names.length === 0) {
      ctx.fillStyle = '#333333';
      ctx.font = '8px Arial';
      ctx.fillText('No staff on floor', PL, namesTop);
    } else {
      const lineH    = rows > 0 ? Math.max(8, Math.min(13, Math.floor(availH / rows))) : 12;
      const fontSize = Math.max(7, lineH - 1);
      ctx.font = `${fontSize}px Arial`;
      names.forEach((name, j) => {
        const col = j % 2;
        const row = Math.floor(j / 2);
        ctx.fillStyle = col === 0 ? '#CCCCCC' : '#AAAAAA';
        ctx.fillText(shortName(name), PL + col * colW, namesTop + row * lineH + lineH - 2);
      });
    }
